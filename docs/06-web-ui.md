...
这意味着仓库里现在已经有了 Web UI 层的一整套“数据、存储、筛选和配置支撑层”，而不再只是 README 里的占位目录。
...
这些都强依赖浏览器宿主。MoonBit 版 phase 06 的目标不是勉强写一个半吊子的 DOM 包装层，而是先把真正稳定的那层能力收完整，让后面的 Web 壳子只是“接线”，不是“重新发明 session / storage / model filter / proxy 逻辑”。

## 测试覆盖

`lib/web_ui` 当前测试覆盖四条主线：
...
也就是说，这一阶段已经把 **Web UI 里可迁移的支撑层做完整了**；缺的不是 session/store/proxy/model 这些基础设施，而是浏览器端渲染与宿主 API 绑定。
